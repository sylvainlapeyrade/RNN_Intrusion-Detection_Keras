from tensorflow._api.v1.keras.layers import Dense, Dropout, LSTM, CuDNNLSTM
...
# Parameters
features_number = four_features
# features_number = eight_features
# features_number = full_features
features_number_values = [four_features, eight_features, full_features]
# dropout_rate = 0.2
...
# units_number = 128
units_number_values = [64, 128, 256]
training_number = 1
epochs = 1
...
def train_model():
    ...
    path = activation_function + '_' + optimizer + '_' + \
        str(epochs) + 'e_' + train_data + '_' + str(len(features_number)-1) + \
        'f_' + str(layer_number) + 'l_' + str(units_number) + 'u_' + \
        str(dropout_rate) + 'd_' + str(batch_size) + \
        'bs_' + str(training_number)
    ...
    # Pile lineaire de couches
    model = Sequential()
    # Ajoute plusieurs couches cachées
    for j in range(layer_number-1):
        model.add(CuDNNLSTM(units=units_number, input_shape=(
    ...
    if(layer_number == 1):
        ...
        model.add(CuDNNLSTM(units=units_number))
        model.add(Dropout(rate=dropout_rate))
    ...
    model.fit(x_train, y_train, epochs=epochs, shuffle=True,
              ...
              use_multiprocessing=True, callbacks=[save_model, tensorboard])
        ...
        for batch_size in batch_size_values:
            for activation_function in activation_function_values:
                for layer_number in layer_number_values:
                    for units_number in units_number_values:
