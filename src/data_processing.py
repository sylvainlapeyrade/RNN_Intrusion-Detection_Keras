...
# Nom et description des features: http://kdd.ics.uci.edu/databases/kddcup99/task.html
feature_name = ["duration", "protocol_type", "service", "flag", "src_bytes", "dst_bytes", "land",
...
# On ne garde que quelques features utiles en accord avec l'article:
# "Applying long short-term memory recurrent neural networks to intrusion detection"
# Les 4 premières feature sont les plus importantes, suivi de 4 autres moins importantes + label
useful_features_plus_label = ['service', 'src_bytes', 'dst_host_diff_srv_rate', 'dst_host_rerror_rate',
                              'dst_bytes', 'hot', 'num_failed_logins', 'dst_host_srv_count', 'label']

# Les 8 features utiles sans la feature label
useful_features = ['service', 'src_bytes', 'dst_host_diff_srv_rate', 'dst_host_rerror_rate',
                   'dst_bytes', 'hot', 'num_failed_logins', 'dst_host_srv_count']

# Type d'attaque: http://kdd.ics.uci.edu/databases/kddcup99/training_attack_types
probe = ['ipsweep.', 'nmap.', 'portsweep.', 'satan.', 'saint.', 'mscan.']
dos = ['back.', 'land.', 'neptune.', 'pod.', 'smurf.', 'teardrop.',
 'apache2.', 'udpstorm.', 'processtable.', 'mailbomb.']
u2r = ['buffer_overflow.', 'loadmodule.', 'perl.', 'rootkit.', 'xterm.', 'ps.', 'sqlattack.']
r2l = ['ftp_write.', 'guess_passwd.', 'imap.', 'multihop.', 'phf.', 'spy.', 
'warezclient.', 'warezmaster.', 'snmpgetattack.', 'named.', 'xlock.', 'xsnoop.', 'sendmail.',
'httptunnel.', 'worm.', 'snmpguess.']

service_values = ['http', 'smtp', 'finger', 'domain_u', 'auth', 'telnet', 'ftp', 'eco_i', 'ntp_u',
 'ecr_i', 'other', 'private', 'pop_3', 'ftp_data', 'rje', 'time', 'mtp', 'link', 'remote_job', 'gopher',
  'ssh', 'name', 'whois', 'domain', 'login', 'imap4', 'daytime', 'ctf', 'nntp', 'shell', 'IRC', 'nnsp',
   'http_443', 'exec', 'printer', 'efs', 'courier', 'uucp', 'klogin', 'kshell', 'echo', 'discard',
    'systat', 'supdup', 'iso_tsap', 'hostnames', 'csnet_ns', 'pop_2', 'sunrpc', 'uucp_path', 'netbios_ns',
     'netbios_ssn', 'netbios_dgm', 'sql_net', 'vmnet', 'bgp', 'Z39_50', 'ldap', 'netstat', 'urh_i', 'X11',
      'urp_i', 'pm_dump', 'tftp_u', 'tim_i', 'red_i', 'icmp', 'http_2784', 'harvest', 'aol', 'http_8001']
...
def process_dataframe(dataframe, name):
    ...
    dataframe = dataframe[useful_features_plus_label]
    ...
    print(' '*6 +'Entrees normales: {:,} soit {}%'.format(normal_data_length,
        round(normal_data_length * 100 / total_data_length, 2)).replace(',', ' '))
    print(' '*6 +'Entrees anormales: {:,} soit {}%'.format(anormal_data_length,
        round(anormal_data_length * 100 / total_data_length, 2)).replace(',', ' '))
    ...
    print('\nDont: Probe: {:,} soit {}%'.format(probe_data_length,
        round(probe_data_length * 100 / total_data_length, 2)).replace(',', ' '))
    print(' '*6 +'DoS: {:,} soit {}%'.format(dos_data_length,
        round(dos_data_length * 100 / total_data_length, 2)).replace(',', ' '))
    print(' '*6 +'U2R: {:,} soit {}%'.format(u2r_data_length,
        round(u2r_data_length * 100 / total_data_length, 2)).replace(',', ' '))
    print(' '*6 +'R2L: {:,} soit {}%'.format(r2l_data_length,
        round(r2l_data_length * 100 / total_data_length, 2)).replace(',', ' '))
